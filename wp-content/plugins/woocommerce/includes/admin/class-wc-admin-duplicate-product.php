...
class WC_Admin_Duplicate_Product 
{
	...
	public function duplicate_product_action() 
	{
		...
		$orig_product_id=$id = isset( $_REQUEST['post'] ) ? absint( $_REQUEST['post'] ) : '';
		...
		if ( ! empty( $post ) ) 
		{
			$duplicate_product_id=$new_id = $this->duplicate_product( $post );
			...
			/***************************Custom Code to copy Makes,Model & Year of Product to duplicate product**********************************/
			
				$duplicte_rowss = mysql_query("INSERT INTO auto_automodels (product_id, make,carYear,model) SELECT '".$duplicate_product_id."', make,carYear,model from auto_automodels where product_id ='".$orig_product_id."'");	
