...
class WC_Admin_Post_Types {
	...
	private function _render_shop_coupon_row_actions( $post, $title ) {
		...
		if ( current_user_can( $post_type_object->cap->delete_post, $post->ID ) ) {
			...
			if ( 'trash' == $post->post_status || ! EMPTY_TRASH_DAYS ) 
			{
				$actions['delete'] = "<a class='submitdelete mkg' title='" . esc_attr( __( 'Delete this item permanently', 'woocommerce' ) ) . "' href='" . get_delete_post_link( $post->ID, '', true ) . "'>" . __( 'Delete Permanently', 'woocommerce' ) . "</a>";
			}
		}
