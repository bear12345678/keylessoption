...
.wc_quick_buy_btn:hover {}'); 
	...
	public function wc_quick_buy_after_add_to_cart_form_add(){
		...
		if($product->is_type( 'simple' ) && in_array('simple',$this->settings['where_to_show'])){
			echo $this->wc_quick_buy_add_form_simple_product($product->id, true, "BUY IT NOW "); 
		} 
		if($product->is_type( 'variable' ) && in_array('variable',$this->settings['where_to_show'])){
			echo $this->wc_quick_buy_add_form_variable_product($product->id, true, "BUY IT NOW"); 
		}
	...
	public function wc_quick_buy_add_form_simple_product($productid,$add_js=true,$btn_name=null){
		...
		$product1 = wc_get_product($orig_id);
		...
		<script>					
			function onquantitychanged() {		
				setTimeout(onquantitychanged, 500);		
				var count=$(".quantity>select").val();
				document.getElementById("buyitnowbtn").innerHTML = "'.$button_name.get_woocommerce_currency_symbol().'" + (' . $product1->get_price() .' * count).toFixed(2);
				document.getElementById("quantity").value = count;
			}
			onquantitychanged();
		</script>
		<input  type="hidden" value="1" name="quantity" id="quantity">
		<input  type="hidden" value="' . $product1->get_price() . '" name="price" id="price">
		<input  type="hidden" value="true" name="quick_buy" />
		<input  type="hidden" name="add-to-cart" value="'.esc_attr($productid).'" />
		<button id="buyitnowbtn" data-productid="'.$productid.'" type="submit" class="wc_quick_buy_product_'.$productid.' quick_buy_'.$productid.'  
                                                                     wc_quick_buy_btn '.$this->settings['class'].'">'.$button_name.get_woocommerce_currency_symbol().$product1->get_price().'</button>';		
		$form .= '</form><br><br><div style="float: right">+ Free Shipping</div>';
