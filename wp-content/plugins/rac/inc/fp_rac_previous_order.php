...
function fp_rac_get_list_of_ids_by_query() {
    if (isset($_POST['rac_order_time'])) {
        ...
        if ("all" == $_POST['rac_order_time']) {
            $args = array('post_type' => 'shop_order', 'posts_per_page' => '-1', "post_status" => function_exists('wc_get_order_statuses') == true ? array_keys(wc_get_order_statuses()) : 'publish', 'fields' => 'ids', 'cache_results' => false);
        } else {
            $args = array('post_type' => 'shop_order', 'posts_per_page' => '-1', 'date_query' => $date_query, "post_status" => function_exists('wc_get_order_statuses') == true ? array_keys(wc_get_order_statuses()) : 'publish', 'fields' => 'ids', 'cache_results' => false);
        }
...
function previous_order_rac() {

    var_dump(RecoverAbandonCart::fp_rac_check_cart_list_manual_recovery('183', '2'));
}

add_action('wp_head', 'previous_order_rac');
